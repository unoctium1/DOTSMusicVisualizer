...
public class FractalSpawnerSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        NativeArray<uint> seeds = new NativeArray<uint>(dataCount, Allocator.TempJob);
            ...
            .ForEach((Entity entity, 
                ...
                in LocalToWorld ltw) =>
                {
                    ...
                    if (spawn.DepthValue > 0 && (spawn.DepthValue == spawn.MaxDepth || random.NextFloat(0.0f, 1.0f) < spawn.SpawnProbability))
                    {
                        ...
                        /*
                        bool setScale = scaleComponents.Exists(entity);
                        float3 scale = new float3(1f);
                        if(setScale){
                            scale = scaleComponents[entity].Value;
                        }
                        */
                        bool setLevel = HasComponent<LevelComponent>(entity);
                        int level = 0;
                        if (setLevel)
                        {
                            level = GetComponent<LevelComponent>(entity).Value;
                        }

                        bool setAmplitude = HasComponent<AmplitudeTagComponent>(entity);

                        for (int i = 0; i < (spawn.isFirst ? 6 : 5); i++)
                        {
                            int maxPrefab = prefabBuffer.Length;
                            Entity meshContainer = prefabBuffer[random.NextInt(0, maxPrefab)].Value;
                            var newFractal = ecb.Instantiate(entityInQueryIndex, meshContainer);
                            ...
                            if (setLevel)
                            {
                                ecb.AddComponent(entityInQueryIndex, newFractal, new LevelComponent
                                {
                                    Value = level
                                });
                            }
                            if (setAmplitude)
                                ecb.AddComponent<AmplitudeTagComponent>(entityInQueryIndex, newFractal);
